...
test.describe('Health Mode Tests', () => {
    test.beforeEach(async ({ page }) => {
        ...
        `});
        await page.click('button[data-category="health"]');
    });

    test('should display Breathing instructions', async ({ page }) => {
        await page.click('button[data-mode="breath"]');
    ...
    test('should change breathing pattern', async ({ page }) => {
        ...
        await expect(page.locator('#time-display')).toHaveText('04:00'); // Assuming default box is 4 mins
    });

    test('should handle Grounding mode', async ({ page }) => {
        await page.click('button[data-mode="grounding"]');
...
test.describe('Sport Mode Tests', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('/');
        await page.click('button[data-category="sport"]');
    });

    test('should configure and run Intervals', async ({ page }) => {
        await page.click('button[data-mode="interval"]');
    ...
    test('should run Stopwatch', async ({ page }) => {
        await page.click('button[data-mode="stopwatch"]');
